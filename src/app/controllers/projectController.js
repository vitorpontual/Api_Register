...
router.put('/:projectId', async (req, res) => {
   try{
      const { title, description, tasks, removeTask } = req.body

      const project = await Project.findByIdAndUpdate(req.params.projectId, {
	 title,
	 description
      }, { new: true });
      project.tasks.map(remove => {
      })
...
router.delete('/:projectId', async (req, res) => {
   try{
      ...
      return res.send({message: "Projeto Removido"})
   }catch(err){
      res.status(400).send({error: 'Error deleteing project'})
   }
})
